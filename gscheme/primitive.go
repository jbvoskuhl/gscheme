...
func installPrimitives(environment Environment) {
	environment.DefineNamed(NewPrimitive("*", minArgs, maxArgs, times))
	environment.DefineNamed(NewPrimitive("+", minArgs, maxArgs, plus))
	environment.DefineNamed(NewPrimitive("-", 1, maxArgs, minus))
	environment.DefineNamed(NewPrimitive("/", 1, maxArgs, divide))
}
